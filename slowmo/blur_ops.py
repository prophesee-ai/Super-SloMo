...
def blur_from_flow(img, flow, bilinear=True, pad_reflect=True):
    ...
    out = torch.zeros(img.shape, dtype=torch.float, device=img.device)
    device = img.device
    ...
    if on_gpu:
        ...
        _cuda_kernel_blur_from_flow[grid_dim, block_dim](*cu_args, bilinear, pad_reflect, 1)
    else:
        args = [v.numpy() for v in args]
        _cpu_kernel_blur_from_flow(*args, bilinear, pad_reflect, 1)
    return out


@numba.jit
def _cpu_kernel_blur_from_flow(img, out, flow, bilinear, pad_reflect, f):
    height, width = img.shape[:2]
                ...
                if bilinear:
                    for lim_y in (math.floor(uy), math.ceil(uy)):
                        for lim_x in (math.floor(ux), math.ceil(ux)):
                            ...
                            for c in range(3):
                                out[y,x,c] += float(img[lim_y, lim_x, c]) * weight / n
                else:
                    lim_y = round(uy)
                    lim_x = round(ux)
                    for c in range(3):
                        out[y,x,c] += float(img[lim_y, lim_x, c]) / n



@cuda.jit()
def _cuda_kernel_blur_from_flow(img, out, flow, bilinear, pad_reflect, f):
    y, x = cuda.grid(2)
    height, width = img.shape[:2]
